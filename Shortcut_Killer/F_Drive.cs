...
{
    public partial class F_Drive : Form
    {
        public F_Drive()
        {
        ...
        private void F_Drive_Load(object sender, EventArgs e)
        {
            StreamWriter writer = new StreamWriter(@"C:\Picra\F.bat", true);
            writer.WriteLine(@"del F:\*.lnk");
            ...
            writer.WriteLine(@"del F:\x.mpeg");
            writer.Close();
            new StreamWriter(@"C:\Picra\F.ini").Close();
            try
            {
                ...
                writer3.WriteLine(@"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            attrib -s -h -r -a /S /D F:\*");
        ...
        public void formClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (File.Exists(@"F:\cure2.bat"))
            {
                File.Delete(@"F:\cure2.bat");
            }
        }
           ...
           if (File.Exists(@"C:\Picra\F.ini"))
            {
               ...
               int drivestatus= zero.validateDriveStatus(@"F:\");
               ...
               if (drivestatus == 0) //possible reason a drive may have been remove
               {
                ...
                else if (this.FBar.Value == 0x7d0)
                {
                    ...
                    string[] strArray = Directory.GetFiles(@"F:\", "*.exe", SearchOption.TopDirectoryOnly);
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        if (info2.Length < 0xdbba0L)
                        {
                            ...
                            if ((((str3.ToString() == str5.ToString()) || (str5.ToString() == str4.ToString())) || ((str3.ToString() == str5.ToString()) && (str3.ToString() == str4.ToString()))) || (str3.ToString() == str4.ToString()))
                            {
                                ...
                                try
                                {
                                    if (Directory.GetDirectories(@"F:\" + fileNameWithoutExtension).Length == 0)
                                    {
                                        splash.shortcutResult.Items.Add(fileNameWithoutExtension.ToString());
                                        Directory.Delete(@"F:\" + fileNameWithoutExtension);
                                    }
                                    goto Label_048A;
                                }
                                catch (Exception)
                    ...
                    if (this.sumT == 0)
                    {
                        ProcessStartInfo info3 = new ProcessStartInfo(@"C:\Picra\F.bat")
                        {
                            UseShellExecute = false,
                            CreateNoWindow = true,
                            RedirectStandardOutput = true,
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            if (this.FBar.Value == 160)
            {
                try
                {
                    if (Directory.Exists(@"F:\589485658"))
                    {
                        string[] files = Directory.GetFiles(@"F:\589485658");
                    ...
                    Directory.Delete(@"F:\589485658");
                }
                catch (Exception)
                {
                }
            }
            if (this.FBar.Value == 300)
            {
                ProcessStartInfo info2 = new ProcessStartInfo(@"C:\Picra\F.bat")
                {
                    UseShellExecute = false,
                    CreateNoWindow = true,
                    RedirectStandardOutput = true,
                ...
                string[] strArray3 = Directory.GetFiles(@"F:\", "*.lnk", SearchOption.TopDirectoryOnly);

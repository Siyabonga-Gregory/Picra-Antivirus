...
{
    public partial class H_Drive : Form
    {
        public H_Drive()
        {
        ...
        public void formClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (File.Exists(@"H:\cure2.bat"))
            {
                File.Delete(@"H:\cure2.bat");
            }
        }
        ...
        private void H_Drive_Load(object sender, EventArgs e)
        {
            StreamWriter writer = new StreamWriter(@"C:\Picra\H.bat",true);
            writer.WriteLine(@"del H:\*.lnk");
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                int drivestatus = zero.validateDriveStatus(@"H:\");
                ...
                else if (this.HBar.Value == 0x7d0)
                {
                    ...
                    string[] strArray = Directory.GetFiles(@"H:\", "*.exe", SearchOption.TopDirectoryOnly);
                    ...
                    for (int i = 0; i < length; i++)
                    {
                        ...
                        if (info2.Length < 0xdbba0L)
                        {
                            ...
                            if ((((str3.ToString() == str5.ToString()) || (str5.ToString() == str4.ToString())) || ((str3.ToString() == str5.ToString()) && (str3.ToString() == str4.ToString()))) || (str3.ToString() == str4.ToString()))
                            {
                                ...
                                try
                                {
                                    int num4 = Directory.GetDirectories(@"H:\" + fileNameWithoutExtension).Length;
                                    ...
                                    if (num4 == 0)
                                    {
                                        ...
                                        Directory.Delete(@"H:\" + fileNameWithoutExtension);
                                    }
                                    goto Label_049B;
                                }
                                catch (Exception)
                            ...
                            try
                            {
                                ...
                                File.Delete(strArray[i]);
                                splash.shortcutResult.Items.Add(strArray[i].ToString());
                                int num5 = Directory.GetDirectories(@"H:\" + str7).Length;
                                writer.WriteLine(Path.GetFileName(strArray[i].ToString() + ".exe"));
                                if (num5 == 0)
                                {
                                    splash.shortcutResult.Items.Add(str7.ToString());
                                    Directory.Delete(@"H:\" + str7);
                                }
                            }
                            catch (Exception)
                            {
                    ...
                    if (this.sumT == 0)
                    {
                        ProcessStartInfo info3 = new ProcessStartInfo(@"C:\Picra\H.bat")
                        {
                            UseShellExecute = false,
                            CreateNoWindow = true,
                            RedirectStandardOutput = true,
        ...
        private void timer2_Tick(object sender, EventArgs e)
        {
            ...
            if (this.HBar.Value == 160)
            {
                try
                {
                    if (Directory.Exists(@"H:\589485658"))
                    {
                        string[] files = Directory.GetFiles(@"H:\589485658");
                    ...
                    Directory.Delete(@"H:\589485658");
                }
                catch (Exception)
                {
                }
            }
            if (this.HBar.Value == 300)
            {
                ProcessStartInfo info2 = new ProcessStartInfo(@"C:\Picra\H.bat")
                {
                    UseShellExecute = false,
                    CreateNoWindow = true,
                    RedirectStandardOutput = true,
                ...
                string[] strArray3 = Directory.GetFiles(@"H:\", "*.lnk", SearchOption.TopDirectoryOnly);
